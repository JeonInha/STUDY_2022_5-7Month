...
class Manage {
	...
	private void printList(Book[] shelf) { // 모든 도서 목록 출력
		for (Book b : shelf) { // for-each문 사용
			printBookSummary(b);
		}
	}

	private Book selectBooktoNum() { // 책넘버로 도서 선택
		System.out.print("선택하려는 책 번호 입력");
		int num=sc.nextInt();
		while (num < 1 || num > 5) {
	...
	private void printBookSummary(Book s) { // 한 권의 도서 정보를 한 줄로 간략하게 출력
		System.out.printf(" %d | %s - %d원 \n", s.getNum(), s.getName(), s.getPrice());
	}

	// Book class의 toString 메소드 - 한 권의 도서 정보 상세 출력
	...
	private void sortOf() { // 정렬한 배열 출력
		System.out.print("1: 가격 낮은 순 / 2: 가격 높은 순 / 3: 도서번호 순  ");
		int select = sc.nextInt();
		if (select == 1) {
			printList(sortOfprice());
		} else if (select == 2) {
			printList(sortOfpriceR());
		} else
	...
	private void AddBook() { // 책추가 동작메소드
		System.out.println("<도서 추가>");
		shelf = makeAddArr(); // new 배열을 기존배열에 덮어씌우기
		editBook(shelf[shelf.length - 1]); // 책 정보 넣기
		System.out.println("도서 추가 완료. ");
	}

	private Book[] makeAddArr() { // 한 칸 늘어난 새 배열 출력
		Book[] copy = Arrays.copyOf(shelf, shelf.length + 1); // 기존 배열과 같지만 길이가 하나 많은 새 배열을 복사해서 만듦.
		copy[copy.length - 1] = new Book(null, null, null, null, 0, copy.length); // 하나 늘어난 변수에 빈 책 넣기
		return copy;
	}
	...
	public void manage() {
		...
		while (true) {
			...
			if (num == 1) {
				...
				printList(shelf);
				System.out.println(selectBooktoNum());
				num = 0;
